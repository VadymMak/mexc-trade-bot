...
export function useSSEQuotes(symbols: string[]) {
  ...
  const handleMessage = useCallback(
    (e: MessageEvent<string> & { event?: string }) => {
      try {
        ...
        if (process.env.NODE_ENV === "development") console.warn("[SSE] malformed frame", err);
      }
    },
  ...
  useEffect(() => {
    if (!joined) {
      if (process.env.NODE_ENV === "development") console.debug("[SSE] skip (no symbols)");
      return;
    }

    if (process.env.NODE_ENV === "development") console.debug("[SSE] connect symbols=", joined);
    const dispose = openMarketStream(joined.split(","), intervalMs, handleMessage);

    return () => {
      if (process.env.NODE_ENV === "development") console.debug("[SSE] close");
      dispose();
    };
